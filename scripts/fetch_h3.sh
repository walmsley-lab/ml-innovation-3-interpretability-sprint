...
gcloud compute ssh "$VM" --zone "${DSI_ZONE:-us-west1-a}" --command \
...
gcloud compute scp "$VM:~/h3_results.tgz" /tmp/h3_results.tgz --zone "${DSI_ZONE:-us-west1-a}"
