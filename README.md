...
numbers were computed from. `scripts/audit_claims.py` recomputes every headline
figure from it, so with the artifacts present a reader can verify the ledger
without re-training; without them, the numbers must be regenerated by re-running.
No public artifact snapshot is published yet.

Every prediction artifact is hashed before the outcomes it predicts exist:
