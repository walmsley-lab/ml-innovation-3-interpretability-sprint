# Deferred work — "Return After X"

Ideas that are worth doing and are **not on the current critical path**. None
of these is discarded. Each carries an explicit trigger, and nothing here is
started before its trigger fires.

The operating principle this file exists to protect:

> Kill the most dangerous alternative explanation first. Preserve promising
> side paths, but do not let them consume the time needed to establish whether
> the central claim survives.

**Current critical path:** checkpoint persistence → V2 preflight (P1–P7) →
6-seed G1 scout → 12-seed expansion → H2.3 ablation. In parallel once frozen:
the sprint latent-state experiment.
...
*Cheap setup now.* The efficiency metrics are already frozen in `STATUS.md`
